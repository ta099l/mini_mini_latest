		...
		// return int
{
	...
	if (cmd->outfile)
	{
		int fd_out = open(cmd->outfile,
				cmd->append ? O_WRONLY | O_CREAT | O_APPEND : O_WRONLY | O_CREAT | O_TRUNC,
				0644);
...
{
	(void)as;
	int fd = open("/tmp/minishell_heredoc_tmp.txt", O_RDWR | O_CREAT | O_TRUNC,
			0644); // open a file
	if (fd == -1)
	{
...
void	execute_commands(t_all *as, t_command *cmd_list, t_envp *env)
{
	...
	while (cmd_list)
	{
		...
		{
			if (built_in(cmd_list) && c == 1 && (!cmd_list->infile
					&& !cmd_list->outfile && !cmd_list->heredoc)) // add the
			{                                                                                                   
			...
			else
			{
				...
				else
				{
					...
					if (num_forked_children == max_children_capacity)
					{
						...
						pid_t *temp_pids = realloc(as->child_pids,
								max_children_capacity * sizeof(pid_t));
							// forbidden cmd
						if (!temp_pids)
						{
							perror("realloc");
