...
int						split_input(t_all *as, char *input, t_token **token,
							t_tmptoken *tmp);
int						is_parameter(char c);
int						parameter_token(t_all *as, char *input, int i,
							t_tmptoken *tmp);
...
void					remove_env_var_utils(t_envp *env, char **new_env,
							int j);
char					*search_path_dirs(char *path, char *cmd);

// Expander
int						expand_var(t_all *as, t_token *token, t_envp *cp_envp,
							int heredoc_expander);
int						process_token(t_all *as, t_token *token,
							t_envp *cp_envp);
int						process_dollar(t_all *as, t_token *token, int *i,
							t_envp *cp_envp);
char					*join_before_env(t_token *token, char *env_value,
							int start);
int						join_after_and_replace(t_token *token, char *var_value,
							int end, int len);
int						expand_variable(t_token *token, char *env_value,
							int start, int end);
int						extract_variable(t_all *as, char *value, int *i,
							char **var);
int						handle_exit_status(t_all *as, t_token *token, int *i);
int						handle_special_dollar(t_all *as, t_token *token,
							int *i);
int						process_token_heredoc(t_all *as, t_token *token,
							t_envp *cp_envp);

// Quotes
int						is_quoted(t_token *token);
int						closing_qoutes(char *input, int i);
int						find_closing_quote(char *input, int i, int flag,
							char quote_type);
int						handle_quotes(t_all *as, char *input, int i,
							t_tmptoken *tmp);
void					replace_qoutes(t_token *token, int i, int j, int q);
void					remove_quotes(t_token *token);
int						handle_single_quotes(const char *value, int *i,
							int in_double);
int						toggle_double_quote(int ch, int in_double);

// Commands
t_command				*new_command(t_all *as);
int						split_cmds(t_all *as, t_token *token,
							t_command **cmd_list);
...
void					execute_exit_utils(char **args, t_all *as,
							int exit_status);
...
void					if_true(char **args, t_envp *env, char *equal_sign,
							int i);
...
void					setup_environment(int argc, char **argv, char **envp,
							t_all **as);
